 ...
 */
Instrumental::Instrumental(string& title, string& artist, map<string, int> tags,
...
{
	return calcTagScore(query) + _calcBpmScore(query)
			+ _calcInstrumentMatchScore(query);
}
/**
...
{
	cout << _title << "\t" << calcScore(query) << "\t" << "performed by: "
			<< _artist << endl;
}
